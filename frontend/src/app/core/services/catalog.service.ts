...
export class CatalogService {
  ...
  getMyListings(page = 0, size = 24): Observable<PageResponse<Product>> {
    return this.http
      .get<PageResponse<Product>>(`${API_BASE_URL}/api/products/me`, {
        params: new HttpParams().set('page', page).set('size', size)
      })
      .pipe(catchError(() => of(this.buildEmptyPage({ page, size }))));
  }

  deleteProduct(productId: number): Observable<void> {
    return this.http.delete<void>(`${API_BASE_URL}/api/products/${productId}`);
  }
