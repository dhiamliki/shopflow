...
import { Order, OrderStatus, PaymentMethod, SellerOrder } from '../models/commerce.models';
...
export class OrdersService {
  ...
  readonly sellerOrders = signal<SellerOrder[]>([]);
  ...
  loadSellerOrders(): Observable<SellerOrder[]> {
    if (!this.session.isSeller() && !this.session.isAdmin()) {
      this.sellerOrders.set([]);
      return of([]);
    }

    return this.http.get<SellerOrder[]>(`${API_BASE_URL}/api/orders/seller`).pipe(
      tap((orders) => this.sellerOrders.set(orders)),
      catchError(() => {
        this.sellerOrders.set([]);
        return of([]);
      })
  ...
  placeOrder(addressId: number, paymentMethod: PaymentMethod): Observable<Order> {
    if (!this.session.isCustomer()) {
  ...
  updateOrderStatus(orderId: number, status: OrderStatus): Observable<Order> {
    return this.http.put<Order>(`${API_BASE_URL}/api/orders/${orderId}/status`, { status });
  }
}
