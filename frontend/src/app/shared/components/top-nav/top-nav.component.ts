          ...
          @if (session.isAuthenticated()) {
            <div class="hidden items-center gap-3 lg:flex">
              <a
                ...
                class="flex items-center gap-3 rounded-md px-2 py-1.5 text-sm font-semibold text-white"
              >
                <span
                ...
                <span class="leading-tight">
                  ...
                  @if (mode() === 'seller') {
                    <span class="text-xs font-medium text-zinc-400">Seller hub</span>
                  }
                </span>
              </a>

              @if (mode() === 'seller' && session.isSeller() && !session.isAdmin()) {
                <a [routerLink]="['/store', session.user()?.id]" class="text-xs font-medium text-zinc-400 hover:text-white">
                  View Store
                </a>
              }
            </div>
