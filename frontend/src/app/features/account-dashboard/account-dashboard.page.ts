  ...
  template: `
    <div class="space-y-6">
      <app-section-heading
        ...
        subtitle="Your order activity and recent spending from your real account."
      />
        ...
        <section class="space-y-5">
          ...
          <app-panel-card title="Recent Orders" subtitle="Track the latest updates on your recent purchases.">
            @if (recentOrders().length) {
              ...
              <app-empty-state icon="package" title="No orders yet" message="Place your first order to see activity here." />
            }
          </app-panel-card>
        </section>
              ...
              <div class="space-y-4">
                @for (item of recommendations(); track item.id) {
                  <div class="flex items-center gap-4 rounded-[22px] border border-white/8 bg-white/[0.03] p-4">
                    <div class="sf-product-media-well h-16 w-16 overflow-hidden rounded-[12px]">
                      <img class="sf-product-card-image h-full w-full" [src]="imageUrl(item.imageUrls[0])" [alt]="item.name" />
                    </div>
