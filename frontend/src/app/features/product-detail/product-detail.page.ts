        ...
        <div class="product-detail-grid">
          <section class="min-w-0 space-y-6">
            ...
            <div class="panel-dark overflow-hidden p-3 sm:p-4">
              <div
                class="sf-product-gallery-stage relative aspect-square w-full overflow-hidden rounded-md"
              >
                @if (selectedImage() && !selectedImageFailed()) {
                  <img class="sf-product-detail-image" [src]="selectedImage()" [alt]="p.name" (error)="onSelectedImageError()" />
                } @else {
                ...
                @for (image of galleryImages(); track image; let imageIndex = $index) {
                  ...
                  >
                    <img
                      ...
                      [alt]="p.name"
                      loading="lazy"
                      (error)="onThumbnailError(imageIndex)"
                    />
                  </button>
                }
              </div>
            ...
            <div class="space-y-7">
              ...
              <div class="space-y-6">
                ...
                <div class="flex flex-wrap items-center gap-3 pt-1 text-base">
                  <span class="h-2.5 w-2.5 rounded-full" [ngClass]="availableStock() > 0 ? 'bg-emerald-400' : 'bg-rose-400'"></span>
                  <span [ngClass]="availableStock() > 0 ? 'text-emerald-300' : 'text-rose-300'">
                    {{ availableStock() > 0 ? 'In stock' : 'Out of stock' }}
                  </span>
                  <span class="text-zinc-500">|</span>
                  <span class="text-zinc-400">{{ availableStock() }} units available</span>
                </div>
              ...
              <div class="mt-7 border-t border-white/8 pt-7">
                <div class="space-y-6 text-sm">
                  ...
                  <div class="flex items-center justify-between">
                    ...
                    <span class="text-zinc-200">{{ availableStock() }} units</span>
                  </div>
                </div>

                @if (p.variants.length) {
                  <div class="mt-7 space-y-3 border-t border-white/8 pt-6">
                    <p class="text-sm text-zinc-400">Select variant</p>
                    <div class="grid gap-2">
                      @for (variant of p.variants; track variant.id) {
                        <button
                          type="button"
                          class="button-secondary min-h-10 w-full justify-between px-3 text-sm"
                          [ngClass]="selectedVariantId() === variant.id ? 'border-emerald-400/35 bg-emerald-500/10 text-emerald-200' : ''"
                          (click)="selectVariant(variant.id)"
                        >
                          <span>{{ variant.attributeName }}: {{ variant.attributeValue }}</span>
                          <span class="text-xs text-zinc-400">{{ variant.stock }} in stock</span>
                        </button>
                      }
                    </div>
                  </div>
                ...
                @if (variantSelectionError()) {
                  <p class="mt-4 text-sm text-rose-300">{{ variantSelectionError() }}</p>
                }
...
export class ProductDetailPageComponent {
  ...
  readonly variantSelectionError = signal('');
  ...
  readonly selectedVariant = computed(() => {
    ...
    if (!product || !product.variants.length || this.selectedVariantId() === null) {
      return null;
    }
    return product.variants.find((variant) => variant.id === this.selectedVariantId()) ?? null;
  });
  readonly availableStock = computed(() => {
    ...
    const fallbackVariant = product.variants.find((variant) => variant.stock > 0) ?? product.variants[0];
  ...
  constructor() {
    effect(() => {
      ...
      this.variantSelectionError.set('');
    });
  }
  ...
  onThumbnailError(imageIndex: number): void {
  ...
  addToCart(): void {
    ...
    if (hasVariants && !selectedVariant) {
      this.variantSelectionError.set('Please select a variant before adding this product to cart.');
      return;
    }
    if (this.availableStock() < 1) {
      this.variantSelectionError.set('This variant is currently out of stock.');
      return;
    }
    this.variantSelectionError.set('');
