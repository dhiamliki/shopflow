            ...
            <div class="panel-dark space-y-7 p-6 sm:p-7">
              <section class="space-y-4">
                ...
                @if (savedAddresses().length) {
                  <div class="space-y-3">
                    <p class="text-sm text-zinc-400">Saved addresses</p>
                    <div class="grid gap-3">
                      @for (address of savedAddresses(); track address.id) {
                        ...
                        >
                          <p class="text-sm font-medium text-white">{{ address.street }}</p>
                    ...
                    <button type="button" class="button-secondary min-h-10 px-4 text-sm" (click)="chooseNewAddress()">
                      Use a new address
                    </button>
                  </div>
                }

                @if (useNewAddress() || !savedAddresses().length) {
...
export class CheckoutPageComponent {
  ...
  constructor() {
    ...
    effect(() => {
      ...
      if (this.useNewAddress() && this.selectedAddressId() === null) {
        return;
      }
  ...
  placeOrder(): void {
    this.orderError.set('');
    const shouldCreateAddress = this.useNewAddress() || this.selectedAddressId() === null;

    if (shouldCreateAddress && this.checkoutForm.invalid) {
    ...
    const addressId$ = !shouldCreateAddress && this.selectedAddressId()
