...
public class JwtService {
    ...
    private Key getSignInKey() {
        byte[] keyBytes = resolveSecretBytes(secretKey);
        return Keys.hmacShaKeyFor(keyBytes);
    }

    private byte[] resolveSecretBytes(String configuredSecret) {
        if (configuredSecret == null || configuredSecret.isBlank()) {
            throw new IllegalStateException("shopflow.jwt.secret must not be empty");
        }
        try {
            ...
            // Not base64; fall through to digesting raw secret.
        }

        try {
            return MessageDigest.getInstance("SHA-256").digest(configuredSecret.getBytes(StandardCharsets.UTF_8));
        } catch (NoSuchAlgorithmException exception) {
            throw new IllegalStateException("SHA-256 not available for JWT secret derivation", exception);
        }
    }
}
