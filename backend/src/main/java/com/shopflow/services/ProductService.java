...
public class ProductService {
    ...
    public Page<ProductResponse> getSellerProducts(int page, int size) {
        ...
        return productRepository.findBySeller_EmailAndActiveTrue(email, PageRequest.of(page, size, Sort.by(Sort.Direction.DESC, "createdAt")))
    ...
    private void replaceImages(Product product, List<String> imageUrls) {
        ...
        String firstImage = imageUrls.stream()
                .filter(Objects::nonNull)
                .map(String::trim)
        ...
        if (firstImage.startsWith("data:image/")) {
            ParsedImageData parsedImage = parseImageDataUrl(firstImage);
